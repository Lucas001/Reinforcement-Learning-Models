...
class ReinforcementLearning(object):
    ...
    def calculateNextAction(self, crawlerPos, joint_angles):
        ...
        i,j = int(joint_angles[0]/6.0),int(joint_angles[1]/6.0)
...
#22 - Both arms activated -
        ...
        for action in range(0,8):
            ...
            if(fut_state[0] < 0): fut_state[0] = 0
            if(fut_state[1] < 0): fut_state[1] = 0
            if(fut_state[0] > max_ind): fut_state[0] = max_ind
            if(fut_state[1] > max_ind): fut_state[1] = max_ind
    ...
    def calculateLocalQStates(self, fut_state, act_state):
        return (1 - 1/self.matrix_visits[act_state[0]][act_state[1]])*self.matrix_states[act_state[0]][act_state[1]] + 1/self.matrix_visits[act_state[0]][act_state[1]]*(self.reward + self.rl_body_linVelocity/5 + self.discount*self.adjustStateValue(fut_state[0],fut_state[1]))
...
class Physics(Thread):
    ...
    def __create_dynamic_car(self,xpos,ypos,velocity):
        ...
        box=body.CreatePolygonFixture(box=(self.car_x_size/2.0,self.car_y_size/2.0), density=0.05, friction=100000)
    ...
    def __create_bodies(self):
        ...
        self.bodylist.update({"arm1":self.__create_dynamic_car(270,130,0)})
        self.bodylist.update({'arm2':self.__create_dynamic_car(190,130,0)})
